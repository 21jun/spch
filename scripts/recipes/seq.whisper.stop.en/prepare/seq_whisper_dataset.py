...
class STOPSeq2SeqDataset(AbstractAudioDataset):
    ...
    def transform(self, datum):
        ...
        label = self.processor.tokenizer(datum["text"].lower()).input_ids
...
def prepare(cfg, processor):
    ...
    with open(cfg.data.eval_datasets["stop-test"]) as f:
        test_clean_list_of_pairs = yaml.load(f, Loader=yaml.FullLoader)

    eval_datasets["stop-test"] = STOPSeq2SeqDataset(
        test_clean_list_of_pairs["data"],
        cfg.data.audio_root_path,
        processor,
        forward_attention_mask,
        max_sample=cfg.data.max_eval_samples,
    )

    with open(cfg.data.eval_datasets["stop-eval"]) as f:
        dev_clean_list_of_pairs = yaml.load(f, Loader=yaml.FullLoader)

    eval_datasets["stop-eval"] = STOPSeq2SeqDataset(
    ...
    eval_dataloaders = {
        "stop-test": test_clean_dataloader,
        "stop-eval": dev_clean_dataloader,
    }
